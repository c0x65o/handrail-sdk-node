...
const DEFAULT_ASSISTANT_BRIDGE_REQUEST_TIMEOUT_MS = 10000;
const DEFAULT_ASSISTANT_BRIDGE_MAX_RETRIES = 2;
const DEFAULT_ASSISTANT_BRIDGE_RETRY_BASE_DELAY_MS = 100;
const DEFAULT_ASSISTANT_BRIDGE_RETRY_MAX_DELAY_MS = 1000;
const ASSISTANT_BRIDGE_CONTRACT_VERSION = 'v1';
const ASSISTANT_BRIDGE_REQUEST_STATUSES = new Set(['pending', 'needs_clarification', 'accepted', 'cancelled', 'failed']);
const ASSISTANT_BRIDGE_TERMINAL_STATUSES = new Set(['accepted', 'cancelled', 'failed']);
const ASSISTANT_BRIDGE_RETRYABLE_STATUSES = new Set([408, 429, 500, 502, 503, 504]);
const ASSISTANT_BRIDGE_SENSITIVE_KEY_PATTERN = /(?:authorization|cookie|password|passwd|secret|token|credential|private[-_]?key|api[-_]?key)/i;
const ASSISTANT_BRIDGE_CREDENTIALS = new WeakMap();
...
function createAssistantChangeBridgeClient(options = {}) {
  return new HandrailAssistantChangeBridgeClient(options);
}

class HandrailAssistantChangeBridgeError extends Error {
  constructor(message, options = {}) {
    super(message);
    this.name = 'HandrailAssistantChangeBridgeError';
    this.code = options.code || 'assistant_bridge_error';
    this.status = options.status;
    this.retryable = Boolean(options.retryable);
    if (options.response !== undefined) {
      this.response = redactAssistantBridgeValue(options.response);
    }
  }
}

class HandrailAssistantChangeBridgeClient {
  constructor(options = {}) {
    const resolved = normalizeAssistantChangeBridgeOptions(options, process.env);
    ASSISTANT_BRIDGE_CREDENTIALS.set(this, resolved.token);
    this.options = Object.freeze(redactedAssistantBridgeConfig(resolved));
    this._fetch = resolved.fetch;
    this._requestTimeoutMs = resolved.requestTimeoutMs;
    this._maxRetries = resolved.maxRetries;
    this._retryBaseDelayMs = resolved.retryBaseDelayMs;
    this._retryMaxDelayMs = resolved.retryMaxDelayMs;
  }
  ...
  getConfig() {
    return { ...this.options, missingConfig: [...this.options.missingConfig] };
  }

  async discover(input) {
    if (!this.isEnabled()) return null;
    const principal = normalizeAssistantBridgePrincipal(input);
    return this._request('discovery', { method: 'GET', principal, retrySafe: true });
  }

  async submit(input) {
    if (!this.isEnabled()) return null;
    const payload = normalizeAssistantBridgeSubmission(input);
    return this._request('requests', {
      method: 'POST',
      principal: payload,
      payload,
  ...
  async lookup(input) {
    if (!this.isEnabled()) return null;
    const principal = normalizeAssistantBridgePrincipal(input);
    const requestId = requiredAssistantBridgeString(input && (input.request_id || input.requestId), 'request_id');
    return this._request(`requests/${encodeURIComponent(requestId)}`, {
      method: 'GET',
      principal,
      retrySafe: true
    });
  ...
  async clarify(input) {
    ...
    const principal = normalizeAssistantBridgePrincipal(input);
    const requestId = requiredAssistantBridgeString(input && (input.request_id || input.requestId), 'request_id');
    const response = requiredAssistantBridgeString(input && (input.response || input.clarification), 'response');
    const payload = compactAssistantBridgeObject({
      ...principal,
      response,
      title: optionalAssistantBridgeString(input && input.title),
      description: input && input.description === null ? null : optionalAssistantBridgeString(input && input.description),
      parent_feature_id: input && input.parent_feature_id === null
        ? null
        : optionalAssistantBridgeString(input && (input.parent_feature_id || input.parentFeatureId))
    });
    return this._request(`requests/${encodeURIComponent(requestId)}/clarifications`, {
      method: 'POST',
      principal,
      payload,
      retrySafe: false
  ...
  async cancel(input) {
    ...
    const payload = compactAssistantBridgeObject({
      ...principal,
      reason: input && input.reason === null ? null : optionalAssistantBridgeString(input && input.reason)
    });
    return this._request(`requests/${encodeURIComponent(requestId)}/cancel`, {
      method: 'POST',
      principal,
      payload,
      retrySafe: true
  ...
  async _request(path, { method, principal, payload, idempotencyKey, retrySafe }) {
    if (!this.isEnabled()) return null;
    const fetchImpl = this._fetch || globalThis.fetch;
    if (typeof fetchImpl !== 'function') {
      throw new HandrailAssistantChangeBridgeError('Assistant Change Bridge fetch is unavailable.', {
        code: 'assistant_bridge_fetch_unavailable'
      });
    }

    const headers = {
      accept: 'application/json',
      authorization: `Bearer ${ASSISTANT_BRIDGE_CREDENTIALS.get(this)}`,
      'x-handrail-principal-issuer': principal.issuer,
      'x-handrail-principal-subject': principal.subject
    };
    let body;
    ...
    const attempts = retrySafe ? this._maxRetries + 1 : 1;
    let lastError;
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      const controller = typeof AbortController !== 'undefined' ? new AbortController() : null;
      const timeout = controller ? setTimeout(() => controller.abort(), this._requestTimeoutMs) : null;
      try {
        ...
        const responsePayload = text ? parseJsonOrText(text) : null;
        if (!response.ok) {
          const status = Number(response.status) || undefined;
          const retryable = ASSISTANT_BRIDGE_RETRYABLE_STATUSES.has(status);
          const error = new HandrailAssistantChangeBridgeError(
            assistantBridgeErrorMessage(responsePayload, status),
            {
              code: assistantBridgeErrorCode(responsePayload),
              status,
              retryable,
          ...
          lastError = error;
          await assistantBridgeRetryDelay(attempt, response, this);
          continue;
        }
        return assertAssistantBridgeResponseTruth(responsePayload);
      } catch (error) {
        if (error instanceof HandrailAssistantChangeBridgeError) throw error;
        lastError = new HandrailAssistantChangeBridgeError('Assistant Change Bridge request failed.', {
          code: error && error.name === 'AbortError' ? 'assistant_bridge_timeout' : 'assistant_bridge_network_error',
          retryable: true
        });
        if (attempt + 1 >= attempts) throw lastError;
        await assistantBridgeRetryDelay(attempt, null, this);
      } finally {
...
function loadAssistantChangeBridgeConfigFromEnv(env = process.env, overrides = {}) {
  return redactedAssistantBridgeConfig(normalizeAssistantChangeBridgeOptions(overrides, env));
}

function normalizeAssistantChangeBridgeOptions(options = {}, env = process.env) {
  const requestedEnabled = parseEnabled(firstDefined(
    options.enabled,
    env.HANDRAIL_ASSISTANT_BRIDGE_ENABLED
  ));
  const rawApiUrl = stringOrUndefined(firstDefined(
    options.apiUrl,
    options.api_url,
    env.HANDRAIL_ASSISTANT_BRIDGE_API_URL
  ));
  const version = stringOrUndefined(firstDefined(
    options.version,
    env.HANDRAIL_ASSISTANT_BRIDGE_VERSION
  ));
  const projectId = stringOrUndefined(firstDefined(
    options.projectId,
    options.project_id,
    env.HANDRAIL_ASSISTANT_BRIDGE_PROJECT_ID
  ));
  const capabilityId = stringOrUndefined(firstDefined(
    options.capabilityId,
    options.capability_id,
    env.HANDRAIL_ASSISTANT_BRIDGE_CAPABILITY_ID
  ));
  const token = stringOrUndefined(firstDefined(
    options.token,
    env.HANDRAIL_ASSISTANT_BRIDGE_TOKEN
  ));
  const apiUrl = normalizeAssistantBridgeApiUrl(rawApiUrl);
  const missingConfig = [];
  if (!apiUrl) missingConfig.push('apiUrl');
  if (!version) missingConfig.push('version');
  if (!projectId) missingConfig.push('projectId');
  if (!capabilityId) missingConfig.push('capabilityId');
  if (!token) missingConfig.push('token');

  let disabledReason = null;
  if (!requestedEnabled) disabledReason = 'disabled';
  else if (!token) disabledReason = 'missing_token';
  else if (!apiUrl) disabledReason = rawApiUrl ? 'invalid_api_url' : 'missing_api_url';
  else if (version !== ASSISTANT_BRIDGE_CONTRACT_VERSION) disabledReason = 'unsupported_version';
  else if (missingConfig.length) disabledReason = 'incomplete_config';

  return {
    enabled: requestedEnabled && disabledReason === null,
    disabledReason,
    missingConfig,
    apiUrl,
    api_url: apiUrl,
    ...
    token,
    requestTimeoutMs: integerOrDefault(
      firstDefined(options.requestTimeoutMs, options.request_timeout_ms, options.fetchTimeoutMs),
      DEFAULT_ASSISTANT_BRIDGE_REQUEST_TIMEOUT_MS
    ),
    maxRetries: nonNegativeIntegerOrDefault(
      firstDefined(options.maxRetries, options.max_retries),
      DEFAULT_ASSISTANT_BRIDGE_MAX_RETRIES
    ),
    retryBaseDelayMs: nonNegativeIntegerOrDefault(
      firstDefined(options.retryBaseDelayMs, options.retry_base_delay_ms),
      DEFAULT_ASSISTANT_BRIDGE_RETRY_BASE_DELAY_MS
    ),
    retryMaxDelayMs: nonNegativeIntegerOrDefault(
      firstDefined(options.retryMaxDelayMs, options.retry_max_delay_ms),
      DEFAULT_ASSISTANT_BRIDGE_RETRY_MAX_DELAY_MS
    ),
    fetch: typeof options.fetch === 'function' ? options.fetch : undefined
  };
}

function normalizeAssistantBridgeApiUrl(value) {
...
function redactedAssistantBridgeConfig(options) {
  return {
...
function requiredAssistantBridgeString(value, field) {
  const normalized = stringOrUndefined(value);
  if (!normalized) {
    throw new HandrailAssistantChangeBridgeError(`Assistant Change Bridge ${field} is required.`, {
      code: `assistant_bridge_${field}_required`
    });
  }
  return normalized;
}

function optionalAssistantBridgeString(value) {
  return value === undefined ? undefined : stringOrUndefined(value);
}

function normalizeAssistantBridgePrincipal(input) {
  return {
    issuer: requiredAssistantBridgeString(input && input.issuer, 'principal_issuer'),
    subject: requiredAssistantBridgeString(input && input.subject, 'principal_subject')
  };
}

function normalizeAssistantBridgeSubmission(input) {
  const principal = normalizeAssistantBridgePrincipal(input);
  const requestedMode = requiredAssistantBridgeString(
    input && (input.requested_mode || input.requestedMode),
    'requested_mode'
  );
  if (!['feature', 'task'].includes(requestedMode)) {
    throw new HandrailAssistantChangeBridgeError('Assistant Change Bridge requested_mode must be feature or task.', {
      code: 'assistant_bridge_requested_mode_invalid'
    });
  }
  const requestedDeliveryCeiling = requiredAssistantBridgeString(
    input && (input.requested_delivery_ceiling || input.requestedDeliveryCeiling),
    'requested_delivery_ceiling'
  );
  if (requestedDeliveryCeiling !== 'intake_only') {
    throw new HandrailAssistantChangeBridgeError('Assistant Change Bridge v1 permits intake_only delivery only.', {
      code: 'assistant_bridge_delivery_escalation_denied'
    });
  }
  return compactAssistantBridgeObject({
    ...principal,
    idempotency_key: requiredAssistantBridgeString(
      input && (input.idempotency_key || input.idempotencyKey),
      'idempotency_key'
    ),
    external_conversation_id: requiredAssistantBridgeString(
      input && (input.external_conversation_id || input.externalConversationId),
      'external_conversation_id'
    ),
    requested_mode: requestedMode,
    requested_delivery_ceiling: requestedDeliveryCeiling,
    title: requiredAssistantBridgeString(input && input.title, 'title'),
    description: input && input.description === null ? null : optionalAssistantBridgeString(input && input.description),
    parent_feature_id: input && input.parent_feature_id === null
      ? null
      : optionalAssistantBridgeString(input && (input.parent_feature_id || input.parentFeatureId)),
    priority: input && input.priority === null ? null : optionalAssistantBridgeString(input && input.priority)
  });
}

function compactAssistantBridgeObject(value) {
  return Object.fromEntries(Object.entries(value).filter(([, entry]) => entry !== undefined));
}

function redactAssistantBridgeValue(value, depth = 0, seen = new WeakSet()) {
  if (value === null || value === undefined || typeof value === 'number' || typeof value === 'boolean') return value;
  if (typeof value === 'string') return value.slice(0, 2000);
  if (depth >= 5) return '[Truncated]';
  if (Array.isArray(value)) return value.slice(0, 50).map((entry) => redactAssistantBridgeValue(entry, depth + 1, seen));
  if (typeof value !== 'object') return undefined;
  if (seen.has(value)) return '[Truncated]';
  seen.add(value);
  const output = {};
  for (const [key, entry] of Object.entries(value).slice(0, 100)) {
    output[key] = ASSISTANT_BRIDGE_SENSITIVE_KEY_PATTERN.test(key)
      ? '[REDACTED]'
      : redactAssistantBridgeValue(entry, depth + 1, seen);
  }
  return output;
}

function assistantBridgeErrorCode(payload) {
  return payload && typeof payload === 'object' && typeof payload.code === 'string'
    ? payload.code
    : 'assistant_bridge_http_error';
}

function assistantBridgeErrorMessage(payload, status) {
  const message = payload && typeof payload === 'object'
    ? firstDefined(payload.error, payload.message)
    : undefined;
  return typeof message === 'string' && message.trim()
    ? message.trim().slice(0, 1000)
    : `Assistant Change Bridge request failed with ${status || 'unknown status'}.`;
}

function assertAssistantBridgeResponseTruth(payload) {
  const request = payload && typeof payload === 'object' && payload.request && typeof payload.request === 'object'
    ? payload.request
    : payload;
  if (!request || typeof request !== 'object' || typeof request.status !== 'string') return payload;
  if (!ASSISTANT_BRIDGE_REQUEST_STATUSES.has(request.status)) {
    throw new HandrailAssistantChangeBridgeError('Assistant Change Bridge returned an unknown request status.', {
      code: 'assistant_bridge_contract_mismatch',
      response: payload
    });
  }
  const expectedTerminal = ASSISTANT_BRIDGE_TERMINAL_STATUSES.has(request.status);
  if (typeof request.terminal !== 'boolean' || request.terminal !== expectedTerminal) {
    throw new HandrailAssistantChangeBridgeError('Assistant Change Bridge returned inconsistent terminal status.', {
      code: 'assistant_bridge_contract_mismatch',
      response: payload
    });
  }
...
async function assistantBridgeRetryDelay(attempt, response, client) {
  const retryAfterHeader = response && response.headers && typeof response.headers.get === 'function'
    ? response.headers.get('retry-after')
    : null;
  const retryAfter = retryAfterHeader !== null && String(retryAfterHeader).trim() !== ''
    ? Number(retryAfterHeader) * 1000
    : NaN;
  const exponential = Math.min(
    client._retryMaxDelayMs,
    client._retryBaseDelayMs * (2 ** attempt)
  );
  const delay = Number.isFinite(retryAfter) && retryAfter >= 0
    ? Math.min(client._retryMaxDelayMs, retryAfter)
    : exponential;
  if (delay <= 0) return;
